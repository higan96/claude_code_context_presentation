...
Claude にとっての**コンテキスト** = 「いま手元にある情報すべて」

- あなたの質問、過去のやりとり、読み込んだファイル、システム指示…
- これらをまとめて **コンテキスト** と呼ぶ
- Claude はコンテキストにある情報**だけ**を使って応答する

人間で言えば「会議中にテーブルに広げた資料」のようなもの。
テーブルに載っていない資料は、どれだけ重要でも参照できない。

→ では、このテーブルにはどれくらいの広さがあるのか？
...
![w:820 center](images/context-usage.png)

カテゴリ別にトークン消費量が見える。まずは一度実行してみるところから
...
| トークン消費の加速 | 利用上限に早く到達する・自動圧縮が頻発する |
...
## ドキュメント化 — 会話からファイルへ逃がす

コンテキストを減らす = 消すことだけではない

```
会話で設計を議論 → 決まったら .md に書き出す → /compact で会話を圧縮
                                                → 次のセッションではファイルを読めばいい
```

プロンプト例：
- 「この設計方針を `docs/architecture.md` に書き出して」
- 「今の議論を `docs/adr/001-auth-strategy.md` にまとめて」

ドキュメントは CLAUDE.md と違い**毎セッション自動で読み込まれない**ため、
書き出した分だけコンテキストの常時負担が減る
...
| 手段 | 向いている内容 | 読み込みタイミング |
|------|---------------|---------|
| CLAUDE.md | ルール・規約 | 毎セッション全文読み込み |
| Auto Memory | 学習・パターン | 毎セッション先頭200行 |
| **ドキュメント** | **設計判断・仕様・調査結果** | **必要時に読む（常時圧迫しない）** |

**判断基準**:
- 毎回 Claude に伝えたいルール → **CLAUDE.md**
- Claude が勝手に学べばいい → **Auto Memory**（管理不要）
- 必要なときだけ参照すればいい → **ドキュメント**
...
| **5** | 議論が固まったらドキュメントに書き出してコンテキストから逃がす |
